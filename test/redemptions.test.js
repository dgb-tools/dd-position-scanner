...
test("13 week-one redemptions: reader txid matches, NUMS collateral inputs == census closes, form matches record presence", () => {
  ...
  for (const f of fx) { const tx = readTx(Buffer.from(f.hex, "hex"), 0); assert.equal(tx.txid, f.txid);
    const nums = tx.vin.filter((v) => isCollateralSpend({ witness: v.witness })).length; assert.equal(nums, f.census.closes, `closes ${f.txid.slice(0, 8)}`);
